...
class FrequencyDomainImputer:
    ...
    def frequency_domain_purification(
        ...
        input_is_frequency_domain: bool = True
    ) -> torch.Tensor:
        """
        在频域中进行扩散净化
        
        Args:
            X_corrupted: 带缺失值的数据（可能是时域或频域）
            mask: 观测值掩码
            input_is_frequency_domain: 输入是否为频域数据
        ...
        with torch.no_grad():
            ...
            if input_is_frequency_domain:
                X_freq = X_corrupted  # 输入已经是频域数据
            else:
                X_freq = dft(X_corrupted)  # 转换到频域
    ...
    def impute(
        ...
        input_is_frequency_domain: bool = True  # 新参数：指示输入是否为频域
        ...
        Args:
            ...
            input_is_frequency_domain: 输入数据是否为频域数据
            
        Returns:
            X_imputed: 补全后的时间序列（与输入域一致）
            mask: 使用的缺失值掩码
        """
        # 移动到正确的设备
        ...
        if mask is None:
            # 如果输入是频域，需要先转换到时域来创建缺失模式
            if input_is_frequency_domain:
                X_time = idft(X.cpu()).to(X.device)
                mask = self.create_missing_mask(X_time, missing_rate, missing_pattern)
                X_missing_time = self.apply_missing_pattern(X_time, mask, fill_value=0.0)
                # 转换回频域进行处理
                X_missing = dft(X_missing_time)
            else:
            ...
            if input_is_frequency_domain:
                X_time = idft(X.cpu()).to(X.device)
                X_missing_time = X_time.clone()
                X_missing_time[mask == 0] = 0.0
                X_missing = dft(X_missing_time)
            else:
                X_missing = X.clone()
